...
import { generateMetaKeywords } from "../keyword-scripts/tamil-keyword-generate";
...
async function englishToHindhiConvert() {
    ...
    for (const { route_name } of medicines) {
        ...
        try {
            ...
            const { rows: tamil_data } = await client.query(
                `SELECT introduction, how_to_use, how_it_works, benefits, side_effects, disease_explanation 
                 FROM medicines_details 
                 WHERE route_name = $1 AND language = $2`,
                [routeName, language]
            );
            const tamilData = tamil_data[0];
            if (tamilData?.introduction && tamilData?.how_to_use && tamilData?.how_it_works &&
                tamilData?.benefits && tamilData?.side_effects && tamilData?.disease_explanation) {
            ...
            const updateQuery = `
                ...
                WHERE route_name = $1 AND language = 'tamil';
            `;
...
`
    try {
        ...
        await createOrAppendFile({ language: "tamil-content", rName: routeName, tToken: response.data.usage.total_tokens })
