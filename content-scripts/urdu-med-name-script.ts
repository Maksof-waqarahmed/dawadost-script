...
import { generateMetaKeywords } from "../keyword-scripts/urdu-keyword-generate";
...
async function englishToHindhiConvert() {
    ...
    for (const { dd_item_code } of medicines) {
        ...
        try {
            ...
            const { rows: urdu_data } = await client.query(
                `SELECT introduction, how_to_use, how_it_works, benefits, side_effects, disease_explanation 
                 FROM medicines_details 
                 WHERE route_name = $1 AND language = 'urdu'`,
                [routeName]
            );
            const urduData = urdu_data[0];
            if (urduData?.introduction && urduData?.how_to_use && urduData?.how_it_works &&
                urduData?.benefits && urduData?.side_effects && urduData?.disease_explanation) {
                console.log("Urdu content already exists for:", routeName);
                continue;
            }
...
`
    try {
        ...
        await createOrAppendFile({ language: "urdu-content", rName: routeName, tToken: response.data.usage.total_tokens })
