...
import { generateMetaKeywords } from "../keyword-scripts/telugu-keyword-generate";
    ...
    for (const { route_name } of medicines) {
        ...
        try {
            ...
            if (!keywordRows[0]?.meta_keywords) {
                try {
                    await generateMetaKeywords(language, routeName);
            ...
            const { rows: language_data } = await client.query(
                `SELECT introduction, how_to_use, how_it_works, benefits, side_effects, disease_explanation 
                 FROM medicines_details 
                 WHERE route_name = $1 AND language = $2`,
                [routeName, language]
            );
            const teluguData = language_data[0];
            if (teluguData?.introduction && teluguData?.how_to_use && teluguData?.how_it_works &&
                teluguData?.benefits && teluguData?.side_effects && teluguData?.disease_explanation) {
                console.log(`${language} content already exists for:`, routeName);
                continue;
            }
            ...
            for (const [key, value] of arr) {
                ...
                try {
                    ...
                    await createOrAppendFile({
                        language: `${language}-invalid-format`,
                        rName: routeName,
                        tToken: 0,
                    });
            ...
            const updateQuery = `
                ...
                WHERE route_name = $1 AND language = 'telugu';
            `;
...
`
    try {
        ...
        await createOrAppendFile({ language: "telugu-content", rName: routeName, tToken: response.data.usage.total_tokens })
