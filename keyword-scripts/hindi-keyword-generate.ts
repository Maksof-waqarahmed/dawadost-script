...
//         try {
//             const { rows: medicineName } = await client.query(`SELECT name FROM medicines_details WHERE route_name = $1 AND language = $2`, [medicineRoute, language]);

//             let keyWords: any = await generateKeyWords({ language: language, name: medicineName[0].name, route_name: medicineRoute });
...
//                 JSON.stringify({
//                     primary: keyWords.primary_keywords?.hindi,
//                     secondary: keyWords.secondary_keywords?.hindi,
//                     mostly_searched: keyWords.mostly_searched_words?.hindi,
//                 }),
//                 medicineRoute,
//                 language,
//             ]);

//             console.log(i++ + ") " + medicineRoute + " updated!");
//         } catch (error: any) {
